...
declare global {
  interface Window {
    toggleNodeExpansion: (depth: number) => void
  }
}
...
export class FamilyTree {
  ...
  constructor(data: Person) {
    ...
    window.toggleNodeExpansion = this.toggleNodeExpansion
  }
    ...
    return ` 
		...
		<div class="gender"> ${genIcon}</div>
			<div class="image-container">
				<img src="${
          img ? img : 'https://www.svgrepo.com/show/483912/person.svg'
        }" alt="Profile Image">
			</div>
			<div class="name">${name}</div>
			<div class="age"> <span><b>${bYr} - ${dYr}</b> </span><span> (${
        age || 0
      } yrs)</span> </div>
    	</div>`
  }
  ...
  private renderNode(person: Person, depth: number = 0): string {
    ...
    const childrenHTML = person.children
    ...
    return `
            <div class="person" style="margin-left: ${margin}px;">
                ...
                <span class="expand-icon ${
                  person.expanded ? 'expand-icon-open' : ''
                } ">${isChild ? expandIcon : ''}</span>
			${
        spouse
